...
with DAG(
    dag_id = '06_delete_entry_data_prod',
    ...
    delete_ml_files = PythonOperator(
                ...
                op_kwargs = {'path': "/opt/airflow/data/mercadolivre/raw/prod/", 'file_extension': "*.json"}
            )
    
    delete_ml_sh_files = PythonOperator(
                task_id='delete_ml_sh_files',
                python_callable = delete_input_files,
                op_kwargs = {'path': "/opt/airflow/data/mercadolivre/shipping_cost_ml/prod/", 'file_extension': "*.json"}
            )
    
    delete_shopee_files = PythonOperator(
            task_id='delete_shopee_files',
            python_callable = delete_input_files,
            op_kwargs = {'path': "/opt/airflow/data/shopee/raw/prod/", 'file_extension': "*.xlsx"}
        )

    delete_sh_ids_files = PythonOperator(
            task_id='delete_sh_ids_files',
            python_callable = delete_input_files,
            op_kwargs = {'path': "/opt/airflow/data/mercadolivre/shipping_cost_ml/prod/", 'file_extension': "*.csv"}
        )
